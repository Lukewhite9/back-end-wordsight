...
const filePatterns = {
  1: glob.glob.sync("wordpairtxts/swapsclean_4_steps_*_difference.txt").concat(glob.glob.sync("wordpairtxts/swapsclean_5_steps_*_difference.txt")),
  2: glob.glob.sync("wordpairtxts/swapsclean_4_steps_*_difference.txt").concat(glob.glob.sync("wordpairtxts/swapsclean_5_steps_*_difference.txt")),
  3: glob.glob.sync("wordpairtxts/swapsclean_5_steps_*_difference.txt"),
  4: glob.glob.sync("wordpairtxts/swapsclean_5_steps_*_difference.txt").concat(glob.glob.sync("wordpairtxts/swapsclean_6_steps_*_difference.txt")),
  5: glob.glob.sync("wordpairtxts/swapsclean_6_steps_*_difference.txt").concat(glob.glob.sync("wordpairtxts/swapsclean_7_steps_*_difference.txt")).concat(glob.glob.sync("wordpairtxts/swapsclean_8_steps_*_difference.txt")),
};
...
async function main() {
  ...
  let completeDays = 0;
  let startDateStr = formatDate(startingDate);
  let endDateStr = "";
  ...
  endDateStr = formatDate(startingDate);

  console.log(`Processed ${completeDays} complete 5-round days from ${startDateStr} to ${endDateStr}`);
}
